...
class CatDogNet(nn.Module):
    def __init__(self):
        ...
        self.max_pool_layer3 = nn.MaxPool2d(kernel_size=2, stride=2)

        # Forth Layer 2xConv and max pool out_shape = (2x2x256)
        self.conv1_layer4 = nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1)
        self.batch_norm1_layer4 = nn.BatchNorm2d(num_features=256)
        self.relu1_layer4 = nn.ReLU()

        self.conv2_layer4 = nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1)
        self.batch_norm2_layer4 = nn.BatchNorm2d(num_features=256)
        self.relu2_layer4 = nn.ReLU()

        self.max_pool_layer4 = nn.MaxPool2d(kernel_size=2, stride=2)

        # Fifth Layer 2xConv and max pool out_shape = (1x1x512)
        self.conv1_layer5 = nn.Conv2d(in_channels=256, out_channels=512, kernel_size=3, stride=1, padding=1)
        self.batch_norm1_layer5 = nn.BatchNorm2d(num_features=512)
        self.relu1_layer5 = nn.ReLU()

        self.conv2_layer5 = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, stride=1, padding=1)
        self.batch_norm2_layer5 = nn.BatchNorm2d(num_features=512)
        self.relu2_layer5 = nn.ReLU()

        self.avg_pool_layer5 = nn.AvgPool2d(kernel_size=2, stride=2)
        ...
        self.cnn_net = nn.Sequential(self.conv1_layer1,
                                     ...
                                     self.relu2_layer3,
                                     self.max_pool_layer3,

                                     self.conv1_layer4,
                                     self.batch_norm1_layer4,
                                     self.relu1_layer4,
                                     self.conv2_layer4,
                                     self.batch_norm2_layer4,
                                     self.relu2_layer4,
                                     self.max_pool_layer4,

                                     self.conv1_layer5,
                                     self.batch_norm1_layer5,
                                     self.relu1_layer5,
                                     self.conv2_layer5,
                                     self.batch_norm2_layer5,
                                     self.relu2_layer5,
                                     self.avg_pool_layer5)
