...
batchGenerator_validation = BatchGenerator(pets_validation, BATCH_SIZE, False,
                                           op=chain([type_cast(dtype=np.float32), add(-127.5), mul(1 / 127.5)]))
...
class CatDogNet(nn.Module):
    def __init__(self):
        ...
        # First Layer 2xConv and Max pool out_Shape = (15x15x32)
        self.conv1_layer1 = nn.Conv2d(in_channels=NUM_CHANNELS, out_channels=32, kernel_size=3, stride=1, padding=1)
        ...
        # second Layer 3xConv and Max pool out_shape = (6x6x64)
        ...
        self.conv3_layer2 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.relu3_layer2 = nn.ReLU()

        self.avg_pool_layer2 = nn.AvgPool2d(kernel_size=2, stride=2)

        # Add all the units into the Sequential layer in exact order
        self.net = nn.Sequential(self.conv1_layer1, self.relu1_layer1, self.conv2_layer1, self.relu2_layer1,
                                 self.max_pool_layer1, self.conv1_layer2, self.relu1_layer2, self.conv2_layer2,
                                 self.relu2_layer2, self.conv3_layer2, self.relu3_layer2, self.avg_pool_layer2)
...
for epoch in range(0, EPOCHS):
    ...
    for batch in batchGenerator_validation:
        predictions = net.predict(batch.data)
